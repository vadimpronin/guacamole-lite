...
class GuacdClient extends EventEmitter {
     ...
     * @param logger
     */
    constructor(guacdOptions, connectionSelector, connectionSettings, logger) {
        super();
        ...
        this.guacdConnection.on('close', (hadError) => {
            // Node.js socket close event passes hadError (boolean), not an error object
            // For graceful closes, we pass undefined (no error)
            this.close(hadError ? new Error('Connection closed unexpectedly') : undefined);
        });

        this.guacdConnection.on('error', (error) => {
            // console.error('********** GuacdClient[' + this.connectionSelector + '] error', error);
    ...
    processConnectionOpen() {
        ...
        this.logger.log(LOGLEVEL.VERBOSE, 'Selecting connection: ' + this.connectionSelector);
